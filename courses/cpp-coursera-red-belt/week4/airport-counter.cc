...
public:
  ...
  size_t Get(TAirport airport) const {
    return data[GetIndex(airport)];
  }
  void Insert(TAirport airport) {
    data[GetIndex(airport)]++;
  }
  ...
  void EraseAll(TAirport airport) {
    data[GetIndex(airport)] = 0;
  }

  static const size_t SIZE = static_cast<size_t>(TAirport::Last_);
  using Item = pair<TAirport, size_t>;
  using Items = array<Item, AirportCounter::SIZE>;

  Items GetItems() const {
    array<Item, SIZE> result;
    for(size_t i = 0; i < data.size(); i++){
      result[i] = {static_cast<TAirport>(i), data[i]};
    }
    return result;
  }

private:
  size_t GetIndex(TAirport airport) const {
