...
public:
  ...
  explicit ConcurrentMap(size_t bucket_count)
    : bucket_count(bucket_count), buckets(bucket_count), mutexes(bucket_count)
  {}

  WriteAccess operator[](const K& key){
    size_t bucket_index = hasher(key) % bucket_count;
    return {lock_guard(mutexes[bucket_index]), buckets[bucket_index][key]};
  }

  ReadAccess At(const K& key) const {
    size_t bucket_index = hasher(key) % bucket_count;
    const V& value = buckets.at(bucket_index).at(key);
    return {lock_guard(mutexes.at(bucket_index)), value};
  }

  bool Has(const K& key) const{
    size_t bucket_index = hasher(key) % bucket_count;
    {
      lock_guard(mutexes.at(bucket_index));
      const MapType bucket = buckets.at(bucket_index);
