...
public:
  ...
  struct Proxy {
    Proxy(Polynomial& obj, size_t degree)
      :poly(obj), degree(degree)
    {}
    ...
    void operator=(T other) {
      ...
      poly.coeffs_[degree] = other;
      poly.Shrink();
    }

    bool operator==(const T other) const {
      return std::as_const(poly)[degree] == other;
    }

    ostream& operator<<(ostream& os) const {
      os <<  std::as_const(poly)[degree];
      return os;
    }
