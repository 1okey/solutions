...
vector<Spending> LoadFromXml(istream& input) {
  ...
  for(const auto& item : doc.GetRoot().Children()){
    result.push_back(
      Spending{item.AttributeValue<string>("category"), item.AttributeValue<int>("amount")
    });
  }
  return result;
}
...
void TestXmlLibrary() {
  // Ð¢ÐµÑÑ‚ Ð´ÐµÐ¼Ð¾Ð½ÑÑ‚Ñ€Ð¸Ñ€ÑƒÐµÑ‚, ÐºÐ°Ðº Ð¿Ð¾Ð»ÑŒÐ·Ð¾Ð²Ð°Ñ‚ÑŒÑÑ Ð±Ð¸Ð±Ð»Ð¸Ð¾Ñ‚ÐµÐºÐ¾Ð¹ Ð¸Ð· Ñ„Ð°Ð¹Ð»Ð° xml.h
